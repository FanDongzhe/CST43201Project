...
//	方法是get的时候可以访问网站二手，此处动态调用数据库将内容显示到二手市场主页
    ...
    public String GoodsDetail2(@PathVariable("id") int id,Model model,HttpSession session)
    {
		...
		//String customerName = customerService.findById(goods.getCustomerid()).getName();
		
		System.out.println("0:"+customerService.findById(goods.getCustomerid()));
		System.out.println("1:"+issueService.findCustomerByGoodsId(id));
